...
# LastEditors: Kai Li
# LastEditTime: 2021-07-14 18:04:16
###
...
class PITLossWrapper(nn.Module):
    ...
    def forward(self, ests, targets, return_ests=False, reduce_kwargs=None, **kwargs):
        ...
        assert pw_loss.shape[0] == targets.shape[0], "PIT loss needs same batch dim as input"
    ...
    def best_perm_from_perm_avg_loss(self, loss_func, ests, targets, **kwargs):
        ...
        loss_set = torch.stack([loss_func(ests[:, perm], targets) for perm in perms], dim=1)
...
if __name__ == "__main__":
    import torch
    from matrix import pairwise_neg_sisdr, singlesrc_neg_sisdr

    ests = torch.randn(10, 32000)
    targets = torch.randn(10, 32000)

    pit_wrapper_1 = PITLossWrapper(singlesrc_neg_sisdr, pit_from="pw_mtx")
    pit_wrapper_2 = PITLossWrapper(singlesrc_neg_sisdr, pit_from="perm_avg")
